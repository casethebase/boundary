...
func (r *ConnectionRepository) AuthorizeConnection(ctx context.Context, sessionId, workerId string) (*Connection, ConnectionStatus, error) {
	const op = "session.(ConnectionRepository).AuthorizeConnection"
	if sessionId == "" {
		return nil, StatusUnspecified, errors.Wrap(ctx, status.Error(codes.FailedPrecondition, "missing session id"), op, errors.WithCode(errors.InvalidParameter))
	}
	connectionId, err := newConnectionId(ctx)
	if err != nil {
		return nil, StatusUnspecified, errors.Wrap(ctx, err, op)
	}
	...
	return &connection, StatusAuthorized, nil
}

// LookupConnection will look up a connection in the repository and return the connection
// with its state. If the connection is not found, it will return nil, nil, nil.
// No options are currently supported.
func (r *ConnectionRepository) LookupConnection(ctx context.Context, connectionId string, _ ...Option) (*Connection, ConnectionStatus, error) {
	const op = "session.(ConnectionRepository).LookupConnection"
	if connectionId == "" {
		return nil, StatusUnspecified, errors.New(ctx, errors.InvalidParameter, op, "missing connectionId id")
	}
	connection := AllocConnection()
	connection.PublicId = connectionId
	...
	if err != nil {
		if errors.IsNotFoundError(err) {
			return nil, StatusUnspecified, nil
		}
		return nil, StatusUnspecified, errors.Wrap(ctx, err, op)
	}
...
func (r *ConnectionRepository) ConnectConnection(ctx context.Context, c ConnectWith) (*Connection, ConnectionStatus, error) {
	...
	return &connection, StatusConnected, nil
}
