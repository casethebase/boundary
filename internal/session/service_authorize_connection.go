...
func AuthorizeConnection(ctx context.Context, sessionRepoFn *Repository, connectionRepoFn *ConnectionRepository,
...
) (*Connection, ConnectionStatus, *AuthzSummary, error) {
	const op = "session.AuthorizeConnection"

	connection, connectionStatus, err := connectionRepoFn.AuthorizeConnection(ctx, sessionId, workerId)
	if err != nil {
		return nil, StatusUnspecified, nil, errors.Wrap(ctx, err, op)
	}
	...
	return connection, connectionStatus, authzSummary, nil
}
